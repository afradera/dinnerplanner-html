...
class DinnerModel {
  ...
  getAllIngredients() {
    //TODO Lab 0
    let allIngredients = [];
    for (const ing of this.menuDishes) { 
        allIngredients.push(ing.ingredients); 
        }
  ...
  getTotalMenuPrice() {
    ...
    let ing = this.getAllIngredients();

   for (const pr of ing) {
        ingredientPrice += pr.price;
    }

    totalPrice = this.nrGuests * ingredientPrice; 
    return totalPrice;
  }
  ...
  addDishToMenu(id) {
    //TODO Lab 0 
    let dsh = this.getDish(id);
    if (this.getSelectedDish(dsh.type) != undefined){
        let id = this.getSelectedDish(dsh.type).id;
        this.removeDishFromMenu(id);
        this.menuDishes.push(dsh);
    } else {
    this.menuDishes.push(dsh);
    }
  }
